...
public class AES {
    ...
    public static byte[] xifraAES(String msg, String clau) throws Exception {
        ...
        IvParameterSpec ivParameterSpec = new IvParameterSpec(iv);      // Crea un objecte que encapsula el iv, per passar-lo al xifratge

        // Genera hash
        MessageDigest messageDigest = MessageDigest.getInstance(ALGORISME_HASH);    // Es crea una instància de l'algoritme SHA-256 per fer un hash de la clau
        byte[] clauHash = messageDigest.digest(clau.getBytes());    // Aplica el hash SHA-256 a la contrasenya
        SecretKeySpec secretKeySpec = new SecretKeySpec(clauHash, ALGORISME_XIFRAT);    // Es crea un objecte que conté la clau AES generada a partir del hash de la contrasenya.
        ...
        System.arraycopy(iv, 0, ivMsgXifrat, 0, msgXifrat.length);
